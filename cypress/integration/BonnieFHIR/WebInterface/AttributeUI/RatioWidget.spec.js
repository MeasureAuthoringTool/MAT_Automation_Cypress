...
describe('Attribute UI: Ratio Widget', () => {
  ...
  it.only('Verify the Ratio Widget', () => {
    uploadTestMeasure()
  ...
  function dragAndDrop () {
    ...
    cy.get('.draggable').eq(40)
        .trigger('mousedown', { which: 1, pageX: 600, pageY: 100 })
        .trigger('mousemove', { which: 1, pageX: 1000, pageY: 100 })
        .trigger('mouseup') 
    cy.get(testPatientPage.criteriaSectionTitle)
        .should('contain.text', 'Medications: MedicationRequest: Moderate Intensity Statin Therapy')
    cy.log('DragAndDropMedications - done')
  }
