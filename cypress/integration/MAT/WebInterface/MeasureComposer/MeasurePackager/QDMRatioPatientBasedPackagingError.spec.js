...
/*
Validate the packaging error when including functions with parameters in the Measure Observations,
that are also contained in the already created group. Packaging should not be allowed
and an error is displayed to the user.
*/

describe('Measure Composer: Measure Packager:QDM Ratio Patient Based Measure Packaging Errors', () => {
  beforeEach('Login', () => {
    login.matLogin()
  })
  afterEach('Log Out', () => {
  ...
  it('QDM Ratio Patient Based Measure: Grouping Validations', () => {

    createMeasureAndGrouping()
    ...
    //Delete function arguments and reset the function data
    ...
    cy.get(measureComposer.functionLeftList).select('Denominator Observation')
    ...
    cy.get(measureComposer.eraseFunctionBtn).click()
    cy.get(measureComposer.functionCQLExpressionEditorInput).type('1', { delay: 50, parseSpecialCharSequences: false })
    cy.get(measureComposer.functionSaveBtn).click()

    helper.visibleWithTimeout(measureComposer.warningMessage)

    cy.get(measureComposer.functionLeftList).select('Numerator Observation')
    cy.get(measureComposer.functionLeftListOptions).eq(1).dblclick()
    cy.get(measureComposer.deleteFunctionArgument).click()
    cy.get(measureComposer.deleteConfirmationYes).click()

    cy.get(measureComposer.eraseFunctionBtn).click()
    cy.get(measureComposer.functionCQLExpressionEditorInput).type('2', { delay: 50, parseSpecialCharSequences: false })
    cy.get(measureComposer.functionSaveBtn).click()
    ...
    //Create Measure Package, should return error
    ...
    cy.get(measureComposer.packageWarningMessage).eq(0).should('contain.text', 'Unable to create ' +
      'measure package. There is an incorrect return type in the following grouping(s):')

    cy.get(measureComposer.packageWarningMessage).eq(1).should('contain.text', 'Grouping 1. Please ' +
      're-save your measure grouping(s) for additional information.')
  })
})


function createMeasureAndGrouping() {
